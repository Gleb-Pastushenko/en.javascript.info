...
The default prototype chain comparison is not executed (regardless of the returned result "true" of "false"). 
```
    For example:    
    ```js run
    class A {}
    ...
    let c = new C();
    alert(c instanceof C); // false: B[Symbol.hasInstance](c) is calles, and logs out 'Custom "hasInstance" called'
    ```
